...
def main(args):
    ...
    logger.info(f"PDB file: {pdb_file}, Cage name: {cage_name}, CAS: {cas_number}")
    ...
    try:
        ...
        logger.info(f"Successfully created cage: {cage.name}")
        print(f"✓ Created cage '{cage.name}': {volume_str}")
        calculator.close()
        return 0
    
    except Exception as e:
        logger.error(f"Error creating cage: {e}", exc_info=True)
